...
while True:
    ...
    if len(lines[3]) > 0:
        for i in range(3, len(lines) - 1):
            process = {'PID': lines[i][0], 'PRIO': lines[i][1], 'USER': lines[i][2], 'DISK_READ': float(lines[i][3]), 'DISK_WRITE': float(lines[i][5]), 'SWAPIN%': float(lines[i][7]), 'IO%': float(lines[i][9]), 'COMMAND': ' '.join(lines[i][11:])}
